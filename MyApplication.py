...
from Threads import AddImagesWorker, Parsing_video, ColmapWorker, Align_according_to_LicensePlate_Worker, \
    SemanticSegmentation_Worker, Splat
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        # self.max_size = 1280
        self.max_size = 1920
        self.temp_result = "output"  # 临时结果文件夹
        self.WORK_DIR = None
        ...
        self.actionSave_as.triggered.connect(self.Save_as_anotherProgram)
        self.actionSplat.triggered.connect(self.Splat)
        # 初始化最近文件菜单
        self.updateRecentFilesMenu()
        self.import_video.triggered.connect(self.addVideo)
        self.import_images.triggered.connect(self.addImageFolder)

    def init_dir(self, WORK_DIR=None, already_save=False):
        # 初始化时设置临时工作目录
        self.WORK_DIR = WORK_DIR
        self.already_save = already_save
        self.save_path = os.path.join(self.WORK_DIR, "save_images")
        self.splat_output = os.path.join(self.WORK_DIR, "splat_output")

        if self.already_save:
            return  # 已经有的工程，下面不能去清除
        ...
        cleardir_ine(os.path.join(self.WORK_DIR, "splat_output"))
    ...
    def add_ui(self):
        ...
        self.actionSplat = QtWidgets.QAction("渲染", self)
        self.menubar.addAction(self.actionSplat)
    ...
    def saveProgram(self):
        try:
            ...
            msg_box = QtWidgets.QMessageBox(QtWidgets.QMessageBox.Information, "提示", "工程已保存",
    ...
    def Save_as_anotherProgram(self):  # 另存为（重新保存一个）
        temp = self.already_save
        self.already_save = False
        f = self.saveProgram()
        if f == -1:
            self.already_save = temp
            return
    ...
    def openProgram(self, open_dir=None):
        ...
        if open_dir is None:
            ...
            if not open_dir:
                print("打开已取消")
                return
        if not os.path.exists(os.path.join(open_dir, "data.json")):
            ...
            if open_dir in self.recent_files_list:
                ...
                self.updateRecentFilesMenu()
                self.settings.setValue('recentFiles', self.recent_files_list)  # 立即保存更改
            return
        self.init_dir(open_dir, already_save=True)  # 初始化工作目录。直接在原工程上操作
    ...
    def addVideo(self):
        if not hasattr(self, 'Parsing_video_worker') or not self.Parsing_video_worker.isRunning():
            ...
            video_path, _ = QtWidgets.QFileDialog.getOpenFileName(self, '选择视频', '',
                                                                  '视频文件 (*.mp4)')
            # 初始化工作目录
            self.init_dir(os.path.join(os.getcwd(), self.temp_result, os.path.basename(video_path)), already_save=False)
            if video_path:
                self.image_paths = []
    ...
    def addImageFolder(self):
        if not hasattr(self, 'add_images_worker') or not self.add_images_worker.isRunning():
            ...
            if folder:
                # 初始化工作目录
                self.init_dir(os.path.join(os.getcwd(), self.temp_result, os.path.basename(folder)), already_save=False)
                # 获取文件夹内所有图像文件
    ...
    def on_thumbnail_clicked_toPoint(self, image_path):
        ...
        if img is None:
            ...
            QtWidgets.QMessageBox.warning(None, "图像读取错误",
                                          f"无法打开图像文件：\n{image_path}\n\n请检查文件路径和完整性。")
            return
        ...
        def onclick(event):
            if event.xdata is not None and event.ydata is not None:
                ...
                if 0 <= y < img.shape[0] and 0 <= x < img.shape[1]:  # 确保坐标在图片范围内
                    ...
                    # plt.draw()
                    #################################
                    #  pixel_to_world函数不做投影处理，输入二维返回三维
    ...
    def calculating_the_scale(self, real_distances):
        ...
        import json
        # 保存到文件
        with open(os.path.join(self.WORK_DIR, "data.json"), "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=4)
    ...
    def export_docx(self):
        ...
        output_path = os.path.join(self.WORK_DIR, "事故报告.docx")
        ...
        current_time = f"{year}年{month}月{day}日{hour}时{minute}分{second}秒"
    ...
    def closeEvent(self, event):
        ...
        # cleardir_ine(os.path.join(os.getcwd(), self.temp_result))# 程序结束清理工作目录
