    ...
    with torch.no_grad():
        ...
        if not skip_train:
            render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline,
                       background, dataset.train_test_exp, separate_sh,args)
...
def render_main(cli_args=None):
    # 如果有外部传参，就临时替换 sys.argv
    old_argv = None
    if cli_args is not None:
        old_argv = sys.argv
        sys.argv = [old_argv[0]] + cli_args  # 保留程序名 + 伪造参数
