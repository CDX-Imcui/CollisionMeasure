            ...
            for y in range(0, image.shape[0], 20):
                for x in range(0, image.shape[1], 20):
                    ...
                    if np.array_equal(pixel, FLOOR_COLOR):
                        ground_pixels.append(
                            (int(round(x * (self.now_size / 640))), int(round(y * (self.now_size / 640)))))  # 记录图像坐标
...
class Splat(QtCore.QThread):
    ...
    def run(self):
        self.log_message.emit('gaussian splatting train...')  # 发送进度信息
        import gaussian_splatting.train as pkg
        pkg.train_main(['-s', self.WORK_DIR, '-m', self.model_path, '--optimizer_type', 'sparse_adam'])

        self.log_message.emit('gaussian splatting render...')  # 发送进度信息
        import gaussian_splatting.render as pkg1
        pkg1.render_main(['-s', self.WORK_DIR, '-m', self.model_path])
        self.finished.emit("渲染完成")
        # # 动态载入包内模块
        # pkg = importlib.import_module("gaussian_splatting.train")  # 相当于执行了import gaussian_splatting.train as pkg
        # train_main = getattr(pkg, "train_main")  # 等价于：train_main = pkg.train_main
        # # 直接调用
